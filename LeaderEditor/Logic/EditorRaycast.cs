...
namespace LeaderEditor
{
    ...
    internal static class EditorRaycast
    {
        public static bool MeshRaycast(Mesh mesh, Ray ray, Matrix4 model, out float t)
        {
            ...
            {
                int index = i * 3;
                Vector3 v0 = (new Vector4(mesh.Vertices[mesh.Indices[index + 0]], 1.0f) * model).Xyz;
                Vector3 v1 = (new Vector4(mesh.Vertices[mesh.Indices[index + 1]], 1.0f) * model).Xyz;
                Vector3 v2 = (new Vector4(mesh.Vertices[mesh.Indices[index + 2]], 1.0f) * model).Xyz;
